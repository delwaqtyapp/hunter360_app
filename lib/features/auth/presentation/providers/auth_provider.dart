...
class AuthState {
  ...
  const AuthState({this.status = AuthStatus.initial, this.user, this.error, this.token, this.rawResponse});
...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Map<String, dynamic> _parseStoredAuth(String raw) {
    ...
    if (raw.trim().startsWith('eyJ')) return {'Token': raw.trim()};
    return {};
  }
  ...
  Future<void> login(String username, String password) async {
    ...
    try {
      ...
      state = state.copyWith(status: AuthStatus.authenticated, user: user, token: token, rawResponse: authJson);
    } catch (e) {
